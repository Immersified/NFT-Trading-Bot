...
const { approveNFT } = require('./functions/list');
...
async function getOSProToken(ethersSigner) {
...
async function listOSPro(order, token) {
  ...
  try {
    ...
    console.error('Error in listOSPro:', error.response.statusText);
    if (error.response) {
      console.error(error.response.data);
    }
  }
...
async function createOSlistingOrder(
  ...
  try {
    ...
    modifiedOrder = await modifyOrder(order, ethersSigner);
...
function modifyOrder(order, ethersSigner) {
...
async function getOSCookies(ethersSigner) {
  // Use CONFIG for constants instead of hardcoding
  let options = {
    method: 'POST',
    url: `${CONFIG.OS_BASE_URL}/__api/graphql/`,
  ...
  const jwt = getJWTFromCookies(combinedCookieHeader, ethersSigner);
...
function getJWTFromCookies(cookies, ethersSigner) {
...
async function refreshOSToken(cookies, ethersSigner) {
  // cookies are supplied by the caller's authenticated session
...
async function listOnOpenseaPro(
  contractAddress,
  tokenId,
  openseaSellPrice,
  ethersSigner
  ...
  const expiryMin = 16;
  let listingPrice = openseaSellPrice; //ETH
  listingPrice = listingPrice.toFixed(9);
...
module.exports = { listOnOpenseaPro };
