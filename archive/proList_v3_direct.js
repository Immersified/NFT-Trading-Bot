...
const CONFIG = {
  ...
  X_NFT_API_KEY: process.env.X_NFT_API_KEY,
  OS_BASE_URL: process.env.OS_BASE_URL,
  OSPRO_BASE_URL: process.env.OSPRO_BASE_URL
};

const HEADERS = {
  'Content-Type': 'application/json',
  'X-NFT-API-Key': CONFIG.X_NFT_API_KEY
};

const jsonProvider = new ethers.JsonRpcProvider(CONFIG.RPC_URL);
const ethersSigner = new ethers.Wallet(CONFIG.PRIVATE_KEY, jsonProvider);

// Authenticate and get OSProToken
async function getOSProToken() {
  const url = `${CONFIG.OSPRO_BASE_URL}/auth/message/${CONFIG.WALLET_ADDRESS}`;
  try {
    ...
    const authUrl = `${CONFIG.OSPRO_BASE_URL}/auth/login`;
    const authResponse = await axios.post(authUrl, data, { headers: HEADERS });
    ...
    console.error('Error in getOSProToken:', error);
    if (error.response) {
...
async function listOSPro(order, token) {
  const url = `${CONFIG.OSPRO_BASE_URL}/opensea/listing`;
  try {
    const response = await axios.post(url, order, { headers: HEADERS });
    if (response.data.order) console.log('Listing Success');
    return response;
  } catch (error) {
    console.error('Error in listOSPro:', error);
    if (error.response) {
      console.error(error.response.data);
    }
  }
...
async function createOSlistingOrder(
  ...
  let options = {
    ...
    headers: {
      ...
      'X-NFT-API-Key': CONFIG.X_NFT_API_KEY,
      'x-auth-address': CONFIG.WALLET_ADDRESS.toLowerCase()
    },
    data: {
...
function modifyOrder(order) {
  ...
  order.parameters.consideration.forEach((item) => {
  ...
  let newFeePercentage = 0.5; // 0.5%

  // Calculate the total amount by summing up all endAmount values in the consideration array
  let totalAmount = order.parameters.consideration.reduce(
    (sum, item) => sum + BigInt(item.endAmount),
    BigInt(0)
  );
  ...
  let feeItem = order.parameters.consideration.find(
    (item) => item.recipient.toLowerCase() === feeRecipient.toLowerCase()
  );

  // Calculate the difference between the original fee and the new fee
  let differenceEnd = BigInt(feeItem.endAmount) - newFee;
  let differenceStart = BigInt(feeItem.startAmount) - newFee;
  ...
  let walletItem = order.parameters.consideration.find(
    (item) => item.recipient === CONFIG.WALLET_ADDRESS
  );
...
async function getOSCookies() {
  ...
  let options = {
    ...
    data: {
      ...
      query: challengeLoginMessageQuery,
      variables: {
        address: CONFIG.WALLET_ADDRESS.toLowerCase()
      }
    }
  };
  let response;
  ...
  const message = response.data.data.auth.loginMessage;
  ...
  //console.log({combinedCookieHeader});
  const jwt = getJWTFromCookies(combinedCookieHeader);
  const expiryInMinutes = decodeJWT(jwt);
  //console.log(expiryInMinutes);
  return combinedCookieHeader;
}

function getJWTFromCookies(cookies) {
  // Define the token's prefix based on the wallet address
  const tokenPrefix = CONFIG.WALLET_ADDRESS.toLowerCase() + '_auth_token=';
...
async function getOSAssestID(contractAddress, tokenId) {
  // Use CONFIG for constants instead of hardcoding
  let options = {
    method: 'POST',
    url: `${CONFIG.OS_BASE_URL}/__api/graphql/`,
...
// Entry function to manage the flow
async function main() {
  const expiryMin = 30;
  const listingPrice = '1'; //ETH
  const contractAddress = '0x00664377c6d191ef4e45380d00f4ce08297855af';
  const tokenId = '1537';

  // Time getOSCookies
  let startTime = new Date();
  const cookies = await getOSCookies();
  let endTime = new Date();
  console.log(`getOSCookies() took ${endTime - startTime} milliseconds.`);

  // Time getOSAssestID
  startTime = new Date();
  const assestID = await getOSAssestID(contractAddress, tokenId);
  endTime = new Date();
  console.log(`getOSAssestID() took ${endTime - startTime} milliseconds.`);

  // Time createOSlistingOrder
  startTime = new Date();
  ...
  endTime = new Date();
  console.log(
    `createOSlistingOrder() took ${endTime - startTime} milliseconds.`
  );

  // Time getOSProToken
  startTime = new Date();
  const token = await getOSProToken();
  endTime = new Date();
  console.log(`getOSProToken() took ${endTime - startTime} milliseconds.`);

  // Time listOSPro
  startTime = new Date();
  const response = await listOSPro(order, token);
  endTime = new Date();
  console.log(`listOSPro() took, ${endTime - startTime} milliseconds.`);
}
